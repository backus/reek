...
RSpec.describe Reek::Smells::DuplicateMethodCall do
  it 'reports the right values' do
    ...
    expect(src).to reek_of(described_class,
                           lines:   [3, 4],
                           context: 'Dummy#double_thing',
                           message: 'calls other.thing 2 times',
                           source:  'string',
                           name:    'other.thing',
                           count:   2)
  end
  ...
  it 'does count all occurences' do
    src = <<-EOS
      class Dummy
        def double_thing(this)
          this.thing
          this.thing
        end

        def double_again(that)
          that.thing
          that.thing
        end
      end
  ...
  context 'with repeated method calls' do
    ...
    it 'reports nested calls' do
      ...
      expect(src).
        to reek_of(:DuplicateMethodCall, name: '@other.thing').
        and reek_of(:DuplicateMethodCall, name: '@other.thing.foo')
    end
